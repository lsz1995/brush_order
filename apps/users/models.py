...
class UserProfile(AbstractUser):#扩展原有user表  setting注册加 AUTH_USER_MODEL='users.UserProfile'
    ...
    balance =models.FloatField(null=True,verbose_name='余额',default=0,help_text='余额')
...
class RechargeOrder(models.Model):
    ...
    user =  models.ForeignKey(UserProfile,verbose_name=u'充值用户',help_text='充值用户')
    money = models.FloatField(verbose_name='充值金额',help_text='充值金额')
    arrival_money = models.FloatField(verbose_name='到账金额',default=0,help_text='到账金额')
    order_number = models.CharField(null=True, blank=True, verbose_name="充值单号",max_length=300,help_text='充值单号')
    add_time = models.DateTimeField(default=datetime.now, verbose_name="添加时间",help_text='创建时间')
    state = models.BooleanField(default=False,verbose_name='处理状态',help_text='审核状态 false代表审核中 true代表审核完成')
...
class WithdrawOrder(models.Model):
    user = models.ForeignKey(UserProfile, verbose_name=u'提现用户',help_text='提现用户')
    money = models.FloatField(verbose_name='提现金额',help_text='提现金额')
    Bank_number =models.CharField(null=True, blank=True, verbose_name="提现银行卡号",max_length=50,help_text='提现银行卡号')
    arrival_money = models.FloatField(verbose_name='到账金额',default=0,help_text='到账金额')
    add_time = models.DateTimeField(default=datetime.now, verbose_name="添加时间",help_text='创建时间')
    state = models.BooleanField(default=False, verbose_name='处理状态',help_text='处理状态')
    name =models.CharField(default='',max_length=6,verbose_name='银行卡持有人姓名',help_text='银行卡持有人姓名')
...
class Store(models.Model):
    ...
    store_ww = models.CharField(max_length=100, default="", verbose_name="店铺旺旺",help_text='店铺旺旺')
    store_name = models.CharField(max_length=100, default="", verbose_name="店铺名称",help_text='店铺名称')
    store_category = models.CharField(max_length=100, default="", verbose_name="店铺类目",help_text='店铺类目')
    store_url = models.URLField(max_length=500,verbose_name='店铺链接',help_text='店铺链接')
    sender_name = models.CharField(max_length=100, default="", verbose_name="发件人",help_text='发件人')
    mobile = models.CharField(max_length=11, default="", verbose_name="电话",help_text='电话')
    ...
    image = models.ImageField(upload_to='store_image', null=True ,blank=True ,default='store_image/default.png', verbose_name="卖家中心图片",help_text='卖家中心图片')
    add_time = models.DateTimeField(default=datetime.now, verbose_name="添加时间",help_text='添加时间')
    state = models.BooleanField(default=False, verbose_name='审核状态',help_text='审核状态')
